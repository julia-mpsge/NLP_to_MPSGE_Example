...
If `subexpressions` is set to `true`, intermediate variables for revenues and costs are created
...
if `subexpressions=false`, but may fail to solve for some parameter values if `subexpressions=true`.

The [`report`](@ref) will *not* work with this model. 
"""
function MCP_model(data::ModelData; subexpressions = false)
